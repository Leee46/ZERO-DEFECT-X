...
│                  FastAPI Telemetry Node (0.0.0.0:8001)                          │
...
│                        FastAPI Intelligence Server (0.0.0.0:8000)                │
│                                                                                 │
│   ┌───────────────────┐    ┌────────────────────┐    ┌──────────────────────┐   │
...
### Step 2: Start Virtual Factory Simulator (Local Port 8001)
```bash
python -m uvicorn simulator.virtual_factory_server:app --host 0.0.0.0 --port 8001 --reload
```

### Step 3: Start ZERO-DEFECT X Backend (Local Port 8000)
```bash
cd backend
python -m pip install -r requirements.txt
# Configure LAPTOP2_URL=http://127.0.0.1:8001 in backend/.env
...
# Virtual Factory Telemetry Endpoint
LAPTOP2_URL=http://127.0.0.1:8001
...
- **Laptop 1 (SCADA & Backend)**: use the host IP reported by `/api/system/network-info` (FastAPI on Port `8000`, React on Port `5173`).
- **Laptop 2 (Virtual Factory Node)**: configure its LAN IP and run the Telemetry API on Port `8001`.
- **Laptop 2 Telemetry Endpoint**: `http://<LAPTOP2-IP>:8001/api/telemetry`.
- Set `LAPTOP2_URL=http://<LAPTOP2-IP>:8001` in Laptop 1 `backend/.env`.
